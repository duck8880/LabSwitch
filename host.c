...
if (j_q->head == NULL ) {
	...
	j_q->tail = j;
	j_q->occ = 1;
	j->next = NULL;	// Chris modified 03182018
}
else {
	(j_q->tail)->next = j;
	j->next = NULL;
...
//printf("debug: Job added. Queue Head = %h, Tail = %h\n", 
//	(int)j_q->head, (int)j_q->tail);
}
...
j_q->occ--;
//printf("debug: Job removed. Queue Head = %h, Tail = %h\n", 
//	(int)j_q->head, (int)j_q->tail);
return(j);
}
...
while(1) {
	...
	for (k = 0; k < node_port_num; k++) { /* Scan all ports */
		...
		if ((n > 0) && ((int) in_packet->dst == host_id)) {
			...
			switch(in_packet->type) {
				...
				// Chris modified++ 03082018	
				/* 
				 * The next three packet types
				 ...
				 * the payload.
				 *
				 * The second type is the packet
				 * which carries the content of the file
				 * in its payload
				 *
				 * The third type is the end packet
				 * which carries the content of the file
				 * in its payload
				 */
	...
	if (job_q_num(&job_q) > 0) {
		...
		//printf("Node %d debug: get new job, type = %d, occ = %d\n", 
		//	host_id, new_job->type, job_q.occ);
		...
		case JOB_SEND_PKT_ALL_PORTS:
			...
			free(new_job->packet);
			free(new_job);
			printf("Node %d debug: JOB_SEND_PKT_ALL_PORTS\n", host_id);
			break;
		...
		case JOB_FILE_UPLOAD_SEND:
			...
			if (dir_valid == 1) {
				...
				if (fp != NULL) {
					...
					new_job2->packet = new_packet;
					job_q_add(&job_q, new_job2);
					printf("Node %d debug: PKT_FILE_UPLOAD_START\n", host_id);
					// Chris modified++ 03082018
					
					// Read the contect of the file until reaching
					//  the end of the file.
					while ((n = fread(string,sizeof(char),
						PKT_PAYLOAD_MAX, fp)) == PKT_PAYLOAD_MAX) {
					/* 
					 * Create intermidiate packet which
					 * has the file name 
					 */
						string[n] = '\0';
					...
					printf("Node %d debug: PKT_FILE_UPLOAD_END. Packet Len = %d\n", host_id, n);
					printf("Node %d debug: Packet content:\n%s\n", host_id, string);
					free(new_job);
		...
		case JOB_FILE_UPLOAD_RECV_START:
			...
			printf("Node %d debug: JOB_FILE_UPLOAD_RECV_START\n", host_id);
			...
			if (dir_valid == 1) {
				...
				name[n] = '\0';
				fp = fopen(name, "w");
				printf("Node %d debug: File name: %s\n", host_id, name);
				/*
				if (fp != NULL) {
					upload_file_open = 1;
				}
		...
		case JOB_FILE_UPLOAD_RECV_END:
			...
			if (dir_valid == 1) {
				...
				if (fp != NULL) {
					 ...
					 * buffer into file
					 */
					printf("node %d debug: JOB_FILE_UPLOAD_RECV_END\n", host_id);
					while (f_buf_upload.occ > 0) {
						n = file_buf_remove(
							&f_buf_upload, 
							string,
